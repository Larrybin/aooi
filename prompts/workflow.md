...
4. `[模式：执行]`：严格按计划编码执行。计划简要（含上下文和计划）存入当前项目根目录的`.codex/plan/任务名.md`。关键步骤后报告进度；仅在阻塞/歧义/风险升级时请求用户反馈或确认。
5. `[模式：优化]`：在 `[模式：执行]` 完成后自动进入。仅分析本次对话产生的相关代码，聚焦冗余与复杂度；默认先做行为等价优化，对可能改变行为的优化先征询确认。
...
1. 默认自主推进；不强制每阶段确认。
2. 当关键信息缺失（影响方案/验收）、关键取舍无法推断、或高风险/破坏性变更时，提出不超过 3 个确认问题。
...
- 为每个步骤指定预期结果
- 使用 Context7 查询新库
- 在项目根目录 `.codex/plan/任务名.md` 中存储执行计划，在继续之前请求用户批准
