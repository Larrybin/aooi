...
function createDeps(
  overrides: Partial<PaymentNotifyFlowDeps> = {}
...
test('payment notify flow 验签成功后写 inbox 并处理 canonical event', async () => {
  ...
  const response = await handlePaymentNotifyRequest({
    provider: 'creem',
    req: new Request('https://example.com/api/payment/notify', {
      method: 'POST',
      headers: {
    ...
    deps: createDeps({
      ...
      recordPaymentWebhookInboxCanonicalEvent: async ({ event: canonical }) => {
  ...
  assert.deepEqual(processed, [
    { inboxId: 'inbox_1', eventType: PaymentEventType.UNKNOWN },
  ]);
});
