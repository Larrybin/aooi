...
test('isAuthSpikeOAuthMockEnabled 仅在显式 true 时启用', () => {
  assert.equal(isAuthSpikeOAuthMockEnabled({}), false);
  assert.equal(
    isAuthSpikeOAuthMockEnabled({ AUTH_SPIKE_OAUTH_MOCK: 'false' }),
    false
  );
  assert.equal(
    isAuthSpikeOAuthMockEnabled({ AUTH_SPIKE_OAUTH_MOCK: 'true' }),
    true
  );
});
...
test('getAuthSpikeOAuthMockConfigs 在非 mock 模式返回空配置', () => {
  assert.deepEqual(getAuthSpikeOAuthMockConfigs({}), {});
});
