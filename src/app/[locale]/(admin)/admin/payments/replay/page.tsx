      ...
      <Main>
        ...
        <Card className="mb-6">
          <CardHeader>
            ...
            <CardDescription>
              Filter payment webhook inbox rows by provider, event id, status, and received time.
            </CardDescription>
          </CardHeader>
          <CardContent>
            <form className="grid gap-4 md:grid-cols-2" method="get">
              ...
              <div className="space-y-2">
                ...
                <Input name="provider" defaultValue={provider} placeholder="stripe / paypal / creem" />
              </div>
              <div className="space-y-2">
                <label className="text-sm font-medium">Event ID</label>
                <Input name="eventId" defaultValue={eventId} placeholder="evt_xxx" />
              </div>
              <div className="space-y-2">
                ...
                >
                  ...
                  <option value={PAYMENT_WEBHOOK_INBOX_STATUS.RECEIVED}>received</option>
                  <option value={PAYMENT_WEBHOOK_INBOX_STATUS.PROCESSED}>processed</option>
                  <option value={PAYMENT_WEBHOOK_INBOX_STATUS.IGNORED_UNKNOWN}>ignored_unknown</option>
                  <option value={PAYMENT_WEBHOOK_INBOX_STATUS.PARSE_FAILED}>parse_failed</option>
                  <option value={PAYMENT_WEBHOOK_INBOX_STATUS.PROCESS_FAILED}>process_failed</option>
                </select>
              </div>
              <div className="space-y-2">
                ...
                <select
                  ...
                  className="border-input bg-background flex h-9 w-full rounded-md border px-3 text-sm"
                >
                  <option value={PAYMENT_WEBHOOK_OPERATION_KIND.REPLAY}>replay</option>
                  <option value={PAYMENT_WEBHOOK_OPERATION_KIND.COMPENSATION}>compensation</option>
                </select>
              </div>
              <div className="space-y-2">
                <label className="text-sm font-medium">Received From</label>
                <Input name="receivedFrom" type="datetime-local" defaultValue={receivedFrom} />
              </div>
              <div className="space-y-2">
                <label className="text-sm font-medium">Received To</label>
                <Input name="receivedTo" type="datetime-local" defaultValue={receivedTo} />
              </div>
        ...
        {previewEnabled ? (
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                {rows.length} rows matched. {executableRows.length} rows have canonical events and can be executed.
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="space-y-3">
                {rows.length === 0 ? (
                  <p className="text-muted-foreground text-sm">No inbox rows matched the current filter.</p>
                ) : (
                  rows.map((row) => (
            ...
            {executableRows.length > 0 ? (
              <CardFooter className="flex-col items-stretch gap-4">
                <form action={submitPaymentWebhookReplayAction} className="space-y-4">
                  <input
                    type="hidden"
                    name="inboxIds"
                    value={JSON.stringify(executableRows.map((row) => row.id))}
                  />
                  <input type="hidden" name="operationKind" value={operationKind} />
