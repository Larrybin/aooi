...
- The no-database build probe does not replace the GitHub Actions Cloudflare matrix yet. Removing the matrix Postgres service is a separate cleanup after the no-DB probe and public/static import allowlist prove that accidental build-time DB dependencies have been eliminated.
