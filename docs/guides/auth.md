...
- CI 中的 `Cloudflare Deploy Acceptance` 不再启动 Postgres service container 或执行 `pnpm db:migrate`。Cloudflare acceptance 只验证配置与构建；schema/migration 配对由独立 guard 负责，生产 migration 仍由本地 release 命令负责。
