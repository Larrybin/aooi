...
test('buildCloudflareSecretsEnv 在 preview 缺本地 CREEM/RESEND secrets 时只警告并跳过', () => {
  const runtimeSettings = {
    secrets: {
      authSharedSecret: true,
      googleOauth: false,
  ...
  try {
    ...
    assert.doesNotMatch(content, /CREEM_API_KEY=/);
    assert.doesNotMatch(content, /CREEM_SIGNING_SECRET=/);
